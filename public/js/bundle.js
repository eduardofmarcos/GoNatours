...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic3VyZmluYmxvb2QiLCJhIjoiY2s3eGliZG81MGRlZjNtcG1wMjdleTNtZyJ9.Pns36jnRSp3GhrMTJbyE4g';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p> Day: ".concat(loc.day, ": ").concat(loc.description, " </p>")).addTo(map); //extends the maps bounds of coordinates
...
var stripe = Stripe('pk_test_qOvopEauzPyFS3vIAicfcsYZ00G03Weztm');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("http://localhost:3000/api/v1/bookings/checkout/".concat(tourId));

          case 3:
            session = _context.sent;
            console.log(session); //2) create process form + charge the credit cards}
...
var loggedBtn = document.querySelector('.nav__el--logout');
var dataForm = document.querySelector('.form.form-user-data');
...
if (loggedBtn) {
  loggedBtn.addEventListener('click', _login.logout);
}
/**** Settings form */
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              tourToGet = e.target.dataset.tourId;
              (0, _stripe.bookTour)(tourToGet);
